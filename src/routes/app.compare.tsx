...
import { explainBestValue, formatMoney, getMedicineSync, getOffers } from "@/services/medicines";
...
const availabilityRank = { in_stock: 0, low_stock: 1, out_of_stock: 2 } as const;
...
const sortOffers = (rows: OfferRow[], key: SortKey) =>
  [...rows].sort((a, b) => {
    ...
    if (key === "availability")
      return (
        availabilityRank[a.listing.availability] - availabilityRank[b.listing.availability] ||
...
function ComparePage() {
  ...
  const compositions = Array.from(
    new Set(ids.map((id) => getMedicineSync(id)?.compositionKey).filter(Boolean) as string[]),
  );
    ...
    <div className="space-y-6">
            ...
            <Button
              ...
              onClick={() => {
                ...
                toast.success("Comparison saved", {
                  description: "It now appears on your dashboard and in your history.",
                });
              }}
            >
      ...
      <section className="rounded-lg border border-border bg-secondary/50 p-5">
        <div className="flex gap-3">
          ...
          <div>
            <h2 className="text-sm font-semibold text-ink">How Medora matches these products</h2>
            <p className="mt-1.5 text-sm text-muted-foreground">
              Products are grouped only when all three match exactly: the same{" "}
              <strong className="font-semibold text-foreground">active ingredient</strong>, the same{" "}
              <strong className="font-semibold text-foreground">strength</strong>, and the same{" "}
              <strong className="font-semibold text-foreground">dosage form</strong>. Matching on
              those three does not mean the products are equally suitable for you — excipients,
              tolerability, manufacturing quality and clinical history all differ, and only a
              pharmacist or prescriber can judge a substitution.
            </p>
      ...
      {mixedComposition && (
        <SafetyNotice tone="warning" title="These products are not equivalent to each other">
          Your selection contains more than one composition, so this table compares different
          products rather than alternatives of the same one. Prices below are still per unit, but do
          not read them as substitution options.
        </SafetyNotice>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-[200px_auto] sm:items-end">
        ...
        <div className="flex items-center gap-2 pb-2">
          <Switch id="cmp-stock" checked={inStockOnly} onCheckedChange={setInStockOnly} />
          ...
          {best && (
            <section className="surface border-primary/30 bg-primary-soft/50 p-6">
              <div className="flex items-start gap-3">
                <TrendingDown className="mt-0.5 size-5 shrink-0 text-primary" aria-hidden />
                <div className="min-w-0">
                  <h2 className="font-display text-lg font-bold text-ink">
                    Lowest unit price: {best.best.medicine.brandName} at {best.best.pharmacy.name}
                  </h2>
                  <p className="mt-1 text-sm text-muted-foreground">
                    {formatMoney(best.savingPerUnit)} less per unit than the most expensive
                    available listing ({Math.round(best.savingPercent)}% lower)
                    {packSaving > 0 && <> — about {formatMoney(packSaving)} on a full pack.</>}
                  </p>
                  ...
                  <p className="mt-3 text-xs text-muted-foreground">
                    &ldquo;Lowest unit price&rdquo; describes this list only. It is not a quality,
                    safety or suitability judgement, and it is not a recommendation to switch.
                  </p>
                  <div className="mt-4">
                  ...
                  <TableBody>
                    {rows.map((o) => (
                      <TableRow key={o.listing.id}>
                        ...
                        <TableCell className="text-sm">{o.medicine.manufacturer}</TableCell>
                        ...
                        <TableCell className="text-sm">{o.listing.packSize}</TableCell>
                  ...
                  <li key={o.listing.id} className="surface p-4">
                    <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
                      ...
                      <div className="text-right">
                        <p className="numeric font-semibold">{formatMoney(o.listing.price)}</p>
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-border pt-3 text-xs text-muted-foreground">
                      ...
                      <span className="numeric">{o.pharmacy.distanceKm} km</span>
                    </div>
      ...
      <SafetyNotice title="Prices are demo data">
        Listings in this environment are sample records with fixed timestamps. Connecting a live
        pricing provider replaces them with verified, timestamped pharmacy data.
      </SafetyNotice>
      <ClinicalDisclaimer />
    </div>
  );

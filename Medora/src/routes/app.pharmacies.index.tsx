...
import { Clock, Copy, List, Map as MapIcon, MapPin, Phone, Star } from "lucide-react";
...
import { EmptyState, IntegrationNotConnected, PageHeader } from "@/components/common/primitives";
      ...
      <div className="mt-auto flex flex-wrap items-center justify-between gap-3 border-t border-border pt-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button asChild size="sm" variant="outline">
            <Link to="/app/pharmacies/$pharmacyId" params={{ pharmacyId: p.id }}>
              View & reserve
            </Link>
          </Button>
        </div>
...
function PharmaciesPage() {
  ...
  const { data, isPending } = useQuery({ queryKey: ["pharmacies"], queryFn: getPharmacies });
  ...
  const list = (data ?? [])
    ...
    .sort((a, b) => {
      ...
      if (sort === "open")
        return Number(isOpenNow(b)) - Number(isOpenNow(a)) || a.distanceKm - b.distanceKm;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        demo
        description="Every pharmacy shown carries a licence identifier so you can confirm it is a legitimate dispensary before you travel."
        actions={
          <div
            role="group"
      ...
      <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_180px] sm:items-end">
        <div className="space-y-1.5">
          <Label htmlFor="pq">Search pharmacies</Label>
          <Input
            id="pq"
            value={q}
            maxLength={80}
        ...
        <section className="surface overflow-hidden">
          <div className="grid-texture flex min-h-72 flex-col items-center justify-center gap-4 border-b border-border p-10 text-center">
            ...
            <div className="max-w-md">
              <p className="font-semibold text-ink">The map view needs a maps provider</p>
              <p className="mt-1.5 text-sm text-muted-foreground">
                Medora will not draw an invented map. Connect a maps provider and this panel becomes
                a live map with pins, travel time and turn-by-turn directions. Until then, the list
                view shows the same pharmacies with real distances from the demo directory.
              </p>
            </div>
          ...
          {isPending ? (
            ...
            <>
              <p className="text-sm text-muted-foreground">
                {list.length} licensed {list.length === 1 ? "pharmacy" : "pharmacies"} in the demo
                directory
              </p>
              <div className="grid gap-4 md:grid-cols-2">
                {list.map((p) => (
